...
# Set detection accuracy threshold direzctly in the code
...
def save_cropped_images(img, boxes, clss, confs, cls_dict):
    """Save cropped images of detections with accuracy percentages in filenames.
...
def loop_and_detect(cam, trt_yolo, conf_th, vis, allowed_classes, cls_dict):
    """Continuously capture images from the camera and perform object detection.
    ...
    while True:
        ...
        # Convert boxes to format suitable for processing (x_min, y_min, x_max, y_max)
        detections = np.array(filtered_boxes, dtype=np.float32)

        if len(detections) == 0:
            print("No detections found, skipping processing for this frame.")
        ...
        # Report detections and calculate heading angles
        for box, cls in zip(filtered_boxes, filtered_clss):
            ...
            print(f"Detected {class_name} at Heading angle (horizontal): {theta_x:.2f} degrees, (vertical): {theta_y:.2f} degrees")

            # Draw the bounding box and the object class name
            cv2.rectangle(img, (x_min, y_min), (x_max, y_max), (0, 255, 0), 2)
            cv2.putText(img, f'{class_name}', (x_min, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
...
def main():
    ...
    # allowed_classes = ['person', 'car', 'boat']  # Set to None to detect all classes, or specify a list of classes.
